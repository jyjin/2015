(function(m, e) {
	...
	m.addLog = function(p, o) {
		m.use("superplus:lib/log", function(q) {
			q.defaultLog(p, o)
		})
	};
	m.fire = function(o, q, p) {
		m.use("superplus:lib/mod_evt", function(r) {
			r.fire(o, q, p)
		})
	};
...
F.module("superplus:lib/sbase", function(c, b, a) {
	var d = b
});
F.use("superplus:lib/sbase", function(b) {
...
F.module("lib/mod_evt", function(f, e, b) {
	var d = {};
...
$.extend({
	localstorage: (function() {
		...
		function j(l) {
			return "_super_" + l.replace(/[_\s]/g, function(m) {
				return m == "_" ? "__" : "_s"
			})
		}
		...
		return {
			set: function(m, o, p, l) {
				var n = this;
				!n._storage && (n._storage = a());
				n._storage.set.apply(n._storage, arguments)
			},
			get: function(l) {
				var m = this;
				!m._storage && (m._storage = a());
...
F.module("lib/log", function(l, t, q) {
	var g = q.base;
	var f = {
			m: 1,
			mod: 1,
		...
		n = s_session.usertype,
		p = "superplus",
		m = s_session.portrait,
		d = s_session.logId || "0",
		k = 0,
		o = {};
	var r = function(w, v) {
	...
	var c = function(u) {
		var w = "imglog__" + (new Date()).getTime(),
			v = window[w] = new Image();
	...
	var e = function(v) {
		var A = r(v._MOD, v._SUBMOD),
			u = v._MOD + "_" + v._EVENT_NAME;
		A.m = u;
		if (o[u]) {
			A.logactid = o[u]
		}
		for (var w in v) {
			...
			if (A[w]) {
				q.warn("logParamConflicted", w + " is conflicted in " + A.m);
				continue
			}
			var y = v[w],
				x = typeof y;
			if (x == "string" || x == "number" || x == "boolean") {
				A[w] = y
			}
		}
		if (s_session.mSid) {
			A.msid = s_session.mSid
		}
		var z = s() + $.url.jsonToQuery(A);
		if (A.logactid == "0200100000" || (A.logactid == "0200100001" && A.clickType == "wordTopic")) {
			c("//www.baidu.com/home/hit/v.gif?" + $.url.jsonToQuery(A))
		}
		q.use("lib/mod_evt", function(B) {
			var C = B.fire("lib/log", "beforeSendLog", {
				modEvt: B,
				logParams: A
			});
			if (C.returnValue) {
	...
	var s = function() {
		return $.url.escapeSSL("http://dj" + (++k % 3) + ".baidu.com/v.gif?")
	};
	var j = /home\/\w+\/(data|submit)\/\w+/;
	q.listen("lib/sbase", "ajaxSuccess", function(v) {
		if (!v.logId) {
			return
		}
		var w = v.url && v.url.match(j),
			u = w && w[0] || "",
			y = u.split("/"),
			x = y && y[1]
	});
...
F.module("superplus:common/user_attr", function(b, d, m) {
	var j = {
		SUBMIT_STATUS: ["/page/submit/userattr", "/xman/submit/superusertips"]
	};
...
F.module("superplus:common/select", function(c, b, a) {
	c("select.css");
	var d = a.base;
	var e = d.Class.create(function(m) {
		...
		var k = function() {
			A.inputMask.bind("mousedown", function(B) {
				A.fire("clickSelect");
				A.input.focus();
				if (A.isshow) {
					w()
			...
			A.inputMask.bind("click", function(B) {
				B.preventDefault();
				B.stopPropagation()
			});
			A.arrow.bind("mousedown", function(B) {
				A.fire("clickSelect");
				A.input.focus();
				if (A.isshow) {
					w()
		...
		var t = function() {
			...
			A.input.bind("keydown", function(C) {
			...
			var B = A.layer.find(".s-select-layer-option");
			A.layer.bind("mouseenter", function(C) {
				$(B[A.selected]).removeClass("current");
				C.preventDefault();
				C.stopPropagation()
			});
			A.layer.bind("mouseleave", function(C) {
			...
			$.each(B, function(C, E) {
				var D = $(E);
				D.bind("mousedown", function(G) {
					f(D.attr("value"), D.html(), C);
					w();
					G.preventDefault();
					G.stopPropagation()
				});
				D.bind("mouseover", function(G) {
					if (A.scroll) {
						return
					}
					j(function(H) {
